...
public class ConversationControllerTest :  IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task StartValidConversation()
    {
        ...
        _conversationStorageMock.Setup(m => m.GetConversation("foo_bar")).ReturnsAsync((Conversation?)null);
        ...
        Assert.Equal(HttpStatusCode.Created, response.StatusCode);
        _conversationStorageMock.Verify(mock => mock.GetConversation("foo_bar"), Times.Once);
    ...
    public async Task StartAlreadyExistingConversation()
    {
        var sendMessageRequest = new SendMessageRequest(Guid.NewGuid().ToString(), "foo", "Hello");
        var startConversationRequest = new StartConversationRequestDto("foo", "bar", sendMessageRequest);
        _conversationStorageMock.Setup(m => m.GetConversation("foo_bar"))
            .ReturnsAsync(new Conversation("foo_bar", "foo", "bar", 1000));
    ...
    public async Task EnumerateConversationsForAUser()
    {
        ...
        {
            new Conversation("foo_bar", "foo", "bar", 1000),
            new Conversation("foo_mike", "foo", "mike", 1001),
            new Conversation("foo_john", "foo", "john", 1002)
        };
